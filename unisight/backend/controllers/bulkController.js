...
export const bulkImportUsers = async (req, res) => {
  try {
    ...
    for (const row of rows) {
      try {
        const { name, email, role, department, studentid } = row;
        
        if (!name || !email || !role)
          throw new Error('Missing required field: name/email/role');
          
        const exists = await User.findOne({ email: email.toLowerCase() });
        if (exists) throw new Error('Email already exists in the system');
        
        // No password provided on bulk upload frontend, so we auto-generate standard
        const defaultPassword = row.password || 'Welcome123!';
