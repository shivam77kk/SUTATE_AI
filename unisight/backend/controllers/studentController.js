 ...
 * Always filters to the LATEST semester/classId so all endpoints are consistent.
 */
async function getLatestContext(studentId) {
  ...
  // Filter marks/attendance to latest semester only
  ...
  // Compute CGPA from marks
  let cgpa = insight?.cgpa ?? null;
  if (!cgpa && marks.length > 0) {
    const totalPossible = marks.length * 160;
    const totalScored = marks.reduce((sum, m) =>
  ...
  const avgAttendance = attendance.length
    ? Math.round(attendance.reduce((sum, row) => sum + (row.percentage || 0), 0) / attendance.length)
    : 0;

  // Risk score from insight - no hardcoded fallbacks
  const dropoutProbability = insight?.dropoutProbabilityScore ?? insight?.dropoutProbability ?? 0;
  const dropoutTier = insight?.dropoutTier ?? insight?.riskLevel ?? 'LOW';

  return { insight, marks, attendance, cgpa, avgAttendance, dropoutProbability, dropoutTier, latestClassId, latestSemester };
}
...
export const getStudentProfile = async (req, res) => {
  try {
    ...
    const ctx = await getLatestContext(studentId);

    const subjects = ctx.attendance.map(a => ({
      subject: a.subject,
    ...
    res.json({
      ...user,
      cgpa: ctx.cgpa ? parseFloat(ctx.cgpa.toFixed(2)) : 0,
      dropoutProbability: ctx.dropoutProbability,
      dropoutTier: ctx.dropoutTier,
      avgAttendance: ctx.avgAttendance,
      classRank: ctx.insight?.classRank ?? '--',
      totalStudents: await User.countDocuments({ role: 'student', department }),
      subjects,
      semester: ctx.insight?.semester ?? user.semester ?? 4,
      department
    });
  } catch (err) {
...
export const getDashboard = async (req, res) => {
  try {
    ...
    if (!ctx.insight) return res.json({ cgpa: 0, dropoutProbability: 0, dropoutTier: 'LOW', recommendations: [], riskReason: null });

    const subjects = [...new Set(ctx.marks.map(m => m.subject))];
    ...
    const marksTrend = exams.map((exam, i) => {
      ...
      for (const sub of subjects) {
        const m = ctx.marks.find(mk => mk.subject === sub);
        row[sub] = m ? (m.scores[scoreKeys[i]] || 0) : 0;
      }
      return row;
    });

    const subjectMarks = ctx.marks.map(m => {
    ...
    res.json({
      cgpa: ctx.cgpa ? parseFloat(ctx.cgpa.toFixed(2)) : 0,
      avgAttendance: ctx.avgAttendance,
      classRank: ctx.insight.classRank || null,
      totalStudents,
      dropoutProbability: ctx.dropoutProbability,
      dropoutTier: ctx.dropoutTier,
      riskReason: ctx.insight.riskReason || null,
      recommendations: ctx.insight.recommendations || [],
      classId: ctx.insight.classId,
      semester: ctx.insight.semester || 4,
      lastAnalysedAt: ctx.insight.generatedAt || ctx.insight.createdAt,
...
export const downloadReport = async (req, res) => {
  ...
  try {
    ...
    doc.text(`Semester: ${ctx.insight?.semester || 'N/A'}`);
    doc.moveDown();

    if (ctx.insight) {
      doc.fontSize(14).fillColor('#6366F1').font('Helvetica-Bold').text('Academic Summary');
      doc.fontSize(11).fillColor('#333');
      doc.text(`Current CGPA: ${ctx.cgpa ? ctx.cgpa.toFixed(2) : 'N/A'}`);
      doc.text(`Risk Level: ${ctx.dropoutTier || 'LOW'}`);
      doc.text(`Risk Score: ${ctx.dropoutProbability}%`);
      doc.text(`Class Rank: ${ctx.insight.classRank ? `#${ctx.insight.classRank}` : 'N/A'}`);
      doc.moveDown();
    }
...
export const chatWithAI = async (req, res) => {
  try {
    ...
    const attendanceSummary = ctx.attendance.map(a => `${a.subject}: ${a.percentage}%`).join(', ');

    const contextPrompt = `
You are a personal academic advisor AI for UniSight university platform.
Student context:
- CGPA: ${ctx.cgpa ? ctx.cgpa.toFixed(2) : 'N/A'}/10.0
- Risk Level: ${ctx.dropoutTier || 'N/A'}
- Risk Score: ${ctx.dropoutProbability}%
- Risk Reason: ${ctx.insight?.riskReason || 'N/A'}
- Marks: ${marksSummary || 'No data'}
- Attendance: ${attendanceSummary || 'No data'}
- Class Rank: #${ctx.insight?.classRank || 'N/A'}
- Semester: ${ctx.insight?.semester || 'N/A'}
Recent conversation:
...
export const getStudyPlan = async (req, res) => {
  try {
    ...
    if (!ctx.insight) return res.json({ plan: [] });
    ...
    if (customSubjects && customSubjects.length > 0) {
      ...
      const topSubjects = ctx.marks.sort((a, b) => {
    ...
    const prompt = `Generate a 4-week study plan JSON starting from ${todayStr} 2026 for: ${subjectContext}. Student CGPA: ${ctx.cgpa ? ctx.cgpa.toFixed(2) : 'N/A'}, Risk: ${ctx.dropoutTier || ctx.insight.riskLevel}.
