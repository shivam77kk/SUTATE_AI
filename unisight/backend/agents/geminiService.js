...
function getModel(generationConfig = {}) {
  const name = MODELS[modelIndex % MODELS.length] || 'gemini-1.5-flash';
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (status === 429) {
        ...
        console.log(`[Gemini Agent] 429 rate-limit. Waiting ${Math.round(delay/1000)}s...`);
...
export async function callGeminiJSON(prompt, fallback = null) {
  try {
    ...
    console.error('[Gemini JSON Fallback] Error:', err.message);
    if (fallback) return fallback;
    // Default safe fallback if none provided
    return { 
      error: true, 
