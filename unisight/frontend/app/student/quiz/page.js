...
function QuizGame({ subject, onBack }) {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center' }}><span className="spinner" style={{ margin: '0 auto', display: 'block' }} /><p style={{ marginTop: 16, color: '#64748b' }}>Generating questions...</p></div>;
    ...
    return (
      <div style={{ textAlign: 'center', padding: 24 }}>
        <svg width={100} height={100} viewBox="0 0 100 100" style={{ margin: '0 auto 20px' }}>
          ...
          <text x="50" y="54" textAnchor="middle" style={{ fontSize: 22, fontWeight: 800, fill: 'white', fontFamily: "'Space Grotesk',sans-serif" }}>{correct}/5</text>
        </svg>
  ...
  const goNext = () => {
    if (current < 4) { setCurrent(c => c + 1); setSelected(null); setChecked(false); }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        {checked && <button onClick={goNext} style={{ flex: 1, padding: 10, borderRadius: 10, background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', color: 'white', fontWeight: 700, cursor: 'pointer', minHeight: 44 }}>{current < 4 ? 'Next →' : 'See results →'}</button>}
      </div>
    </div>
  );
}
  ...
  if (selectedSubject) return (
    <div className="dashboard-content">
      ...
      <div className="chart-container">
        <QuizGame subject={selectedSubject} onBack={() => setSelectedSubject(null)} />
      </div>
    </div>
  );
  ...
  return (
    <div className="dashboard-content">
      ...
      {isLoading ? <CardSkel height={300} /> : (
        <>
          {subjects[0] && (
            <button onClick={() => setSelectedSubject(subjects[0].subject)} style={{ width: '100%', padding: 14, borderRadius: 12, background: 'linear-gradient(135deg,#6366f1,#7c3aed)', border: 'none', color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer', marginBottom: 20, minHeight: 48 }}>
            ...
            {subjects.map((subj, i) => {
              ...
              return (
                <div key={i} className="chart-container" style={{ borderLeft: `3px solid ${c}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                  ...
                  <button onClick={() => setSelectedSubject(subj.subject)} style={{ padding: '8px 16px', borderRadius: 8, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', color: '#818cf8', fontWeight: 600, fontSize: 13, cursor: 'pointer', minHeight: 44 }}>
                    Practice →
                  </button>
                </div>
              );
